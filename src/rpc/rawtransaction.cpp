...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the viacoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    BOOST_FOREACH(const string& name_, addrList) {
        ...
        if (name_ == "data") {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Viacoin address: ")+name_);
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"Vg5iRXkaipLKSYBDJjkV5qa1CE9b1djeHn\"   (string) viacoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"addresses\": [   (json array of string)\n"
            "     \"address\"     (string) viacoin address\n"
            "     ,...\n"
            "  ],\n"
